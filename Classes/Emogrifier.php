...
class Emogrifier
{
    ...
    private function mapAllInlineStylesToHtmlAttributes(\DOMXPath $xPath)
    {
        try {
            ...
            if ($this->debug) {
                throw new \RuntimeException($e->getMessage(), 1508437884, $e);
            }
            return;
        }
    ...
    private function removeImportantAnnotationFromAllInlineStyles(\DOMXPath $xPath)
    {
        try {
            ...
            if ($this->debug) {
                throw new \RuntimeException($e->getMessage(), 1509193363, $e);
            }
            return;
        }
     ...
     *
     * Can indirectly generate a \RuntimeException via method handleXpathError().
     *
     * @param \DOMXPath $xPath
     *
     * @return \DOMNodeList
    ...
    private function normalizeStyleAttributesOfAllNodes(\DOMXPath $xPath)
    {
        try {
            ...
            if ($this->debug) {
                throw new \RuntimeException($e->getMessage(), 1509190518, $e);
            }
            return;
        }
